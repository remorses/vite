...
const hmrPlugin = ({ root, app, server, watcher, resolver, config }) => {
  ...
  const handleJSReload = (watcher.handleJSReload = (
    ...
    if (importers || isHmrAccepted(publicPath, publicPath)) {
      ...
      dirtyFiles.add(utils_1.cleanUrl(publicPath))
      ...
      if (hasDeadEnd) {
        ...
        console.log(
          chalk_1.default.green(`[vite:hmr] `) +
            `${utils_1.cleanUrl(
              file
            )} hot updated due to change in ${relativeFile}.`
        )
        send({
          type: 'multi',
...
function walkImportChain(
  ...
  for (const importer of importers) {
    if (
      utils_1.cleanUrl(importer).endsWith('.vue') ||
      ...
      if (utils_1.cleanUrl(importer).endsWith('.vue')) {
