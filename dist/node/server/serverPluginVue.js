...
const vuePlugin = ({ root, app, resolver, watcher, config }) => {
  ...
  app.use(async (ctx, next) => {
    ...
    if (query.type === 'style') {
      ...
      const id = hash_sum_1.default(utils_1.cleanUrl(publicPath))
  ...
  const handleVueReload = (watcher.handleVueReload = async (
    ...
    const publicPath = utils_1.cleanUrl(resolver.fileToRequest(filePath))
    ...
    const styleId = hash_sum_1.default(utils_1.cleanUrl(publicPath))
    ...
    nextStyles.forEach((_, i) => {
      if (!prevStyles[i] || !isEqualBlock(prevStyles[i], nextStyles[i])) {
        didUpdateStyle = true
        const path = utils_1.appendQuery(publicPath, `type=style&index=${i}`)
        send({
          type: 'style-update',
          path,
          changeSrcPath: path,
...
async function resolveSrcImport(root, block, ctx, resolver) {
  const importee = serverPluginModuleRewrite_1.resolveImport(
    root,
    ctx.url,
    block.src,
    resolver
...
async function compileSFCMain(descriptor, filePath, publicPath, root) {
  ...
  if (descriptor.styles) {
    descriptor.styles.forEach((s, i) => {
      const styleRequest = utils_1.appendQuery(
        publicPath,
        `type=style&index=${i}`
      )
  ...
  if (descriptor.customBlocks) {
    descriptor.customBlocks.forEach((c, i) => {
      ...
      let customRequest = utils_1.appendQuery(
        publicPath,
  ...
  if (descriptor.template) {
    const templateRequest = utils_1.appendQuery(publicPath, `type=template`)
...
async function compileSFCStyle(
  ...
  const resource = utils_1.appendQuery(publicPath, `type=style&index=${index}`)
  cssUtils_1.recordCssImportChain(result.dependencies, resource) // TODO recordCssImportChain should accept only files not public paths
