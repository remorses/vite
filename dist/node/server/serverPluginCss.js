...
const cssPlugin = ({ root, app, watcher, resolver }) => {
  app.use(async (ctx, next) => {
    ...
    if (
      ...
      const id = hash_sum_1.default(ctx.path)
  ...
  function vueStyleUpdate(styleImport) {
    ...
    const index = utils_1.parseWithQuery(styleImport).query.index
    if (!index) {
      console.error(
        new Error(`no index found for vue style path ${styleImport}`)
      )
    }
    const path = utils_1.appendQuery(publicPath, `type=style&index=${index}`)
    console.log(
  ...
  function moduleCssUpdate(filePath, resolver) {
    // bust process cache
    processedCSS.delete(utils_1.cleanUrl(resolver.fileToRequest(filePath)))
  ...
  async function processCss(root, ctx) {
    ...
    const preprocessLang = (ctx.path.match(cssUtils_1.cssPreprocessLangRE) ||
      [])[1]
