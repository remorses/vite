...
const defaultRequestToFile = (publicPath, root) => {
  var _a
  const search =
    (_a = publicPath.match(utils_1.queryRE)) === null || _a === void 0
      ? void 0
      : _a[0]
  const query = querystring_1.default.parse(search ? search.slice(1) : '')
  const { realPath = '' } = query
  if (realPath) {
    return path_1.default.resolve(
      utils_1.cleanUrl(Array.isArray(realPath) ? realPath[0] : realPath)
    )
  }
  // the query is no more necessary
  publicPath = utils_1.cleanUrl(publicPath)
  if (serverPluginModuleResolve_1.moduleRE.test(publicPath)) {
    ...
    if (nodeModule) {
      serverPluginModuleResolve_1.moduleIdToFileMap.set(publicPath, nodeModule)
      return nodeModule
    }
  }
...
const defaultFileToRequest = (filePath, root) => {
  ...
  const cached = serverPluginModuleResolve_1.moduleFileToIdMap.get(filePath)
  if (cached) {
    return cached
  }
  const realPath = pathUtils_1.osAgnosticPath(filePath)
  ...
  const res = pathUtils_1.appendQuery(
    '/' + slash_1.default(relative).replace(/^public\//, ''),
    querystring_1.default.encode({ realPath })
  )
  return res
}
const isFile = (file) => {
  ...
  const resolver = {
    ...
    normalizePublicPath(publicPath) {
      ...
      if (!serverPluginModuleResolve_1.moduleRE.test(cleanPublicPath)) {
        ...
        res = pathUtils_1.mapQuery(res, (q) => {
          var _a
          const queryMatch =
            (_a = publicPath.match(utils_1.queryRE)) === null || _a === void 0
              ? void 0
              : _a[0]
          return {
            ...(queryMatch && querystring_1.default.parse(queryMatch.slice(1))),
            ...q
          }
        })
        return finalize(res)
      ...
      return finalize(
        ['/@modules', scope, name, filePathPostFix].filter(Boolean).join('/') +
          (url_1.default.parse(publicPath).search || '')
      )
    },
    alias(id) {
    ...
    resolveRelativeRequest(importer, importee) {
      ...
      importee = utils_1.cleanUrl(importee)
      ...
      if (importee.startsWith('.')) {
        ...
        realPath = pathUtils_1.osAgnosticPath(
      ...
      const query = querystring_1.default.encode({
        ...querystring_1.default.parse(
          queryMatch ? queryMatch[0].slice(1) : ''
        ),
        ...(realPath && { realPath })
      })
      const pathname =
      ...
      return pathUtils_1.appendQuery(pathname, query)
    },
    ...
    isAssetRequest(filePath) {
      return (
        (assetsInclude && assetsInclude(filePath)) ||
        pathUtils_1.isStaticAsset(filePath)
      )
    }
  }
  return resolver
 ...
 */
function resolveBareModuleRequest(root, publicPath, importer, resolver) {
  var _a
  let id = utils_1
    .cleanUrl(publicPath)
    .replace(serverPluginModuleResolve_1.moduleRE, '')
  let realPath
  ...
  if (pkgInfo) {
    if (!pkgInfo.entry) {
      ...
      isEntry = true
      id = pkgInfo.entry
      realPath = pathUtils_1.osAgnosticPath(pkgInfo.entryFilePath)
    }
  }
  if (!isEntry) {
    ...
    if (deepMatch) {
      ...
      realPath = pathUtils_1.osAgnosticPath(resolveNodeModuleFile(basedir, id))
      if (realPath) {
        const deepPath = id.replace(deepImportRE, '')
        const normalizedFilePath = slash_1.default(realPath)
  ...
  return pathUtils_1.mapQuery(id, () => {
    const ext = path_1.default.extname(utils_1.cleanUrl(id))
    const { query } = pathUtils_1.parseWithQuery(publicPath)
    return {
      ...query,
      ...(!exports.jsSrcRE.test(ext) && { import: '' }),
      ...(realPath && { realPath })
    }
  })
}
...
function resolveNodeModuleFile(root, id) {
  ...
  try {
    const resolved = utils_1.resolveFrom(root, utils_1.cleanUrl(id))
