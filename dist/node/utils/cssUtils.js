...
function isCSSRequest(path) {
  const cleaned = pathUtils_1.cleanUrl(path)
  return (
    path.endsWith('.css') ||
    exports.cssPreprocessLangRE.test(path) ||
    cleaned.endsWith('.css') ||
    exports.cssPreprocessLangRE.test(cleaned)
  )
}
...
async function compileCss(
  ...
  const id = hash_sum_1.default(pathUtils_1.cleanUrl(publicPath))
